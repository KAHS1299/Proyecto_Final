...
form.addEventListener("submit", async (event) => {
    ...
    level.className = "";
    level.textContent = "Calculating...";
    text.textContent = "Sending the selected dataset variables to the prediction API.";
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(result.error || "The prediction API returned an error.");
        }

        level.className = "";
        level.classList.add(`level-${result.level.toLowerCase()}`);
        level.textContent = `Saturation: ${result.level}`;
        text.textContent = `${result.explanation} Reference occupancy: ${result.reference_occupancy}%.`;
        ...
        level.className = "";
        level.textContent = "Prediction unavailable";
        text.textContent = error.message;
        tourists.textContent = "0";
        recommendation.textContent = "Check that Flask was restarted after the dataset update and try again.";
    }
});
